...
### Test Vectors & Regression Testing
To prevent silent regressions from circuit changes or toolchain updates, deterministic test vectors are maintained in `testvectors/`. These vectors capture:
- Known witness inputs
- Expected public outputs
- VK hash for each circuit

CI automatically verifies that circuit outputs match committed test vectors. If you modify circuit logic or upgrade the toolchain:
1. Review the changes carefully
2. Regenerate test vectors: `./scripts/generate_testvectors.sh` (or `.ps1` on Windows)
3. Update deployed contracts if VK changed
4. Commit new test vectors

See [`testvectors/README.md`](./testvectors/README.md) for details and [`WINDOWS_SETUP.md`](./WINDOWS_SETUP.md) for Windows-specific toolchain setup.
