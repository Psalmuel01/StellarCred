...
pub enum DataKey {
    ...
    /// (holder, credential_type) -> identity commitment (public-input field 0
    /// of the verified proof). The commitment is the ZK circuit's binding of
    /// the credential's attributes + issuer salt: it is *stable for a human*
    /// across wallet addresses, which is exactly what a per-app nullifier
    /// needs. Stored under its own key so the change is additive — existing
    /// `ProofRecord` entries decode unchanged (#nullifier).
    Subject(Address, Symbol),
...
impl ProofRegistry {
    ...
    pub fn submit_proof(
        ...
        Self::store_subject(&env, &holder, &credential_type, &public_inputs, 0, expiry);
    ...
    pub fn check_delegated_verification(
        ...
        let now = env.ledger().timestamp();
        let delegation_key = DataKey::Delegation(holder.clone(), verifier, credential_type.clone());
    ...
    // ── Per-app nullifiers (#airdrop / anti-Sybil) ──────────────────────────
    //
    // A *per-app nullifier* is a deterministic, app-scoped pseudonym for the
    // human behind a credential:
    //
    //     nullifier = sha256( identity_commitment || app_scope )
    //
    // `identity_commitment` is public-input field 0 of the verified proof —
    // the circuit's binding of the credential's attributes and the issuer's
    // salt. It is the *same value* no matter which wallet address submits the
    // proof, so two Sybil addresses holding one credential collapse to one
    // nullifier. `app_scope` is an arbitrary byte string chosen by the
    // consuming app (e.g. `b"stellarcred:airdrop:humandrop-2026"`): the hash
    // makes nullifiers for different scopes unlinkable, so a claim in one
    // campaign cannot be correlated with a claim in another.
    //
    // Distribution contracts consume this: "has this human already claimed in
    // this campaign?" — see the `human_airdrop` reference contract.

    /// The identity commitment recorded for `holder`'s `credential_type`
    /// proof, or `None` if no proof was ever submitted (or it predates
    /// commitment recording). Does **not** check validity — use
    /// `app_nullifier`, which does.
    pub fn identity_commitment(
        ...
        env.storage()
            .persistent()
            .get::<_, BytesN<32>>(&DataKey::Subject(holder, credential_type))
    }

    /// The per-app nullifier for `holder`'s `credential_type` claim under
    /// `app_scope`, or `None` when the claim is missing, revoked, expired,
    /// from an untrusted issuer, or below `min_threshold`.
    ///
    /// Returning `None` (rather than panicking) keeps this usable as a cheap
    /// read from both a distributing contract and an off-chain SDK.
    pub fn app_nullifier(
        ...
        let commitment: BytesN<32> = env
            .storage()
            .persistent()
            .get(&DataKey::Subject(holder, credential_type))?;

        let mut preimage = Bytes::new(&env);
        preimage.append(&Bytes::from_array(&env, &commitment.to_array()));
        preimage.append(&app_scope);
    ...
    pub fn revoke_proof(env: Env, holder: Address, credential_type: Symbol) {
        ...
        env.storage()
            .persistent()
            .remove(&DataKey::Subject(holder, credential_type));
    }
    ...
    pub fn revoke_all(env: Env, holder: Address) {
        ...
        for t in types {
            env.storage()
                .persistent()
                .remove(&DataKey::Proof(holder.clone(), t.clone()));
            env.storage()
                .persistent()
                .remove(&DataKey::Subject(holder.clone(), t));
        }
    }
    ...
    /// Record the identity commitment (public-input field 0, at
    /// `field_offset` inside an aggregate proof's packed inputs) that backs a
    /// freshly-verified claim. Written under `DataKey::Subject` so the
    /// existing `ProofRecord` layout — and every deployed record — is
    /// untouched.
    fn store_subject(
        env: &Env,
        holder: &Address,
        ...
        let base = field_offset * FIELD_BYTES;
        let mut arr = [0u8; 32];
        for (i, slot) in arr.iter_mut().enumerate() {
            *slot = public_inputs.get(base + i as u32).unwrap_or(0);
        }
        let key = DataKey::Subject(holder.clone(), credential_type.clone());
        env.storage()
            .persistent()
            .set(&key, &BytesN::from_array(env, &arr));
        Self::bump_ttl(env, &key, expiry);
    }
...
mod test;
 successful verifications so protocols don't re-run the (expensive)
//! UltraHonk verifier on every interaction. A holder proves once; the registry
//! records "this address satisfies credential X until ledger time T". Any gated
//! protocol then makes a single cheap `is_verified` call.
//!
